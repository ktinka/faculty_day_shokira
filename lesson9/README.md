...
```bash
du -ah logs | sort -rhf | head -3; nano biglog.txt; error.logs; ^x; y
```
...
```bash
mkdir project_backup;cp config/*.conf project_backup;
cp -r logs/*.log project_backup;cp errors.txt project_backup;
